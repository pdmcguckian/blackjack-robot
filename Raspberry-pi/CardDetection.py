...
#This file is imported by the main.py file and called when the robot needs to read a card.
#It uses a Tensorflow Lite model which was trained by us on a more powerful computer than the Raspberry Pi
#The code also uses OpenCV to first process the image (outlined in slides and comments)
#The image is then compared to the tflite model to give a reading of the cards value.
#25 readings are taken and the most common value is assigned to a variable which is read by the main.py code
...
class CardDetection():
    def __init__(self, camera):
        ...
        #import and initiate tensorflow lite model
        self.model = tflite.Interpreter(model_path = "/home/pi/BJOS/converted_model.tflite")
        time.sleep(1.5)
        ...
        vals = []
        while len(vals) < 25: #read the card 25 times
            self.prep_frame()
            self.isolate_card()
            self.transform_card()
            self.locate_number()
            self.match_card()
            vals.append(self.card_rank)
            print(self.card_rank)
        
        self.card_rank =max(set(vals), key = vals.count) #find the most common value
        print('Out', self.card_rank)
        self.stream.release()
    ...
    #Image Converted to Grayscare, Blurred & Thresholded
        ...
        #Thresholding
        self.retval, self.threshold = cv2.threshold(self.blur, 30, 255, cv2.THRESH_BINARY)
    ...
    #Corner of image where number is located is isolated and zoomed in by a factor of 4 and prepared
        ...
        #Thresholding
        if self.camera == 2:
            self.retval, self.num_thresh = cv2.threshold(self.num_blur, 170, 255, cv2.THRESH_BINARY)
        
        else:
            self.retval, self.num_thresh = cv2.threshold(self.num_blur, 150, 255, cv2.THRESH_BINARY)

    def match_card(self):
    #test the processed image against the tflite model
        self.test_image = np.array(self.num_thresh, dtype = np.float32).reshape(-1, 55, 35, 1)
        self.test_image = cv2.normalize(self.test_image, None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F).reshape(self.test_image.shape[0], -1)
        self.model.set_tensor(self.input_details[0]['index'], self.test_image)
        self.model.invoke()
        self.output_data = self.model.get_tensor(self.output_details[0]['index'])

        #this value is read by the main code
        self.card_rank = np.argmax(self.output_data) + 1
